...
const App: React.FC = () => {
  ...
  const calculateScore = useCallback(() => {
      let correctCount = 0;
      questions.forEach(q => {
          // If answer is undefined (unanswered), it won't equal correctOptionIndex, so 0 points.
  ...
  const handleSubmitExam = async (forceSubmit = false) => {
      // If not forced (manual click), and answers are incomplete, ask for confirmation
      if (!forceSubmit && Object.keys(answers).length < questions.length) {
          if (!window.confirm("No has respondido todas las preguntas. ¿Estás seguro de finalizar? Las preguntas no respondidas tendrán puntaje cero.")) {
              return;
          }
      }
  ...
  const handleTimeUp = () => {
      alert("¡El tiempo ha terminado! El examen se cerrará y se enviarán las respuestas registradas hasta el momento.");
      // Force submit without confirmation dialog
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
        <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md border-t-4 border-teal-500">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
                ...
                <button 
                    type="submit" 
                    className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3 rounded-lg transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                >
                    Comenzar Evaluación
                </button>
            </form>
            <p className="mt-4 text-xs text-gray-500 text-center">
                Tendrás <strong>10 minutos</strong> para responder 20 preguntas.
            </p>
        </div>
    </div>
  );
  ...
  const renderTesting = () => (
      <div className="container mx-auto px-4 py-8 max-w-4xl relative">
          <Timer durationMinutes={10} onTimeUp={handleTimeUp} />
          ...
          <div className="bg-white rounded-xl shadow-xl p-8 mb-8">
              ...
              <div className="space-y-8">
                  {questions.map((q, index) => (
                      <div key={q.id} className="p-6 bg-teal-50 rounded-lg border border-gray-200 hover:border-teal-300 transition-colors">
